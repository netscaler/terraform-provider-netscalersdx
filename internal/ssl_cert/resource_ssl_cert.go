...
func (r *sslCertResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// file_name authoritative. The SDX form field is the plural "ssl_certs".
	localPath := filepath.Join(data.FileLocationPath.ValueString(), data.FileName.ValueString())
	_, err := r.client.UploadFile(endpoint, "ssl_certs", localPath)
	if err != nil {
...
func (r *sslCertResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	tflog.Debug(ctx, fmt.Sprintf("In Read Method of ssl_cert Resource with Id: %s", resId))
	...
	if err != nil {
		...
		tflog.Warn(ctx, fmt.Sprintf("removing resource ssl_cert: %v from state because it is not present in the remote", data.Id.ValueString()))
		return
	}
...
// Update is a no-op. ssl_cert has no NITRO upload-update endpoint, so any real
...
func (r *sslCertResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	tflog.Debug(ctx, "In Update Method of ssl_cert Resource (no-op; RequiresReplace handles real changes)")
...
func (r *sslCertResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	tflog.Debug(ctx, "In Delete Method of ssl_cert Resource")
